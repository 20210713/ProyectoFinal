...
window.addEventListener("load", event=> {
    callAPI(url, "GET", {}).then( clientes => {
        const elementoListado = document.querySelector("#listado-interacciones")
        ...
        clientes.forEach(cliente => {
            ...
            link.href = `EditInts?id=${cliente.id}`
            link.textContent = `Id: ${cliente.id} | Detalle: ${cliente.note}. | Id de usuario: ${cliente.userId} | Id de cliente: ${cliente.costumerId} | Fecha: ${cliente.createdAt}`
